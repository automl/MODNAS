...
from search_spaces.gpt.blocks.kv_cache import KVCache
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        #print(heads_selected)
        qkv = self.attn(x)*embed_dim_selected*heads_selected*bias_selected
        cos = self.cos[:T]
        sin = self.sin[:T]
        ...
        y = self.scaled_dot_product_attention(q, k, v, mask)*heads_selected

        y = y.reshape(B, T, self.sample_head_size*self.sample_n_head)  # re-assemble all head outputs side by side
        #print(y)
        # output projection
        #print(self.proj(y))
        return self.proj(y)*embed_dim_selected*heads_selected*bias_selected
