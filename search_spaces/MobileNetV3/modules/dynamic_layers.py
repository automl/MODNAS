...
class DynamicMBConvLayer(MyModule):
    def __init__(
        ...
        if max(self.expand_ratio_list) == 1:
            ...
            self.inverted_bottleneck = nn.ModuleDict(
                      {      "conv":
                            DynamicConv2d(
        ...
        self.point_linear = nn.ModuleDict({
                        "conv":
    ...
    def forward(self, x, kernel_max_param, expand_max_param):
        in_channel = x.size(1)
        ...
        if self.inverted_bottleneck is not None:
            x = self.inverted_bottleneck["conv"](x) * expand_max_param
            x = self.inverted_bottleneck["bn"](x) * expand_max_param
            x = self.inverted_bottleneck["act"](x)
        x = self.depth_conv["conv"](x) * kernel_max_param * expand_max_param
        x = self.depth_conv["bn"](x) * expand_max_param
        x = self.depth_conv["act"](x) * expand_max_param
        if self.use_se:
            x = self.depth_conv["se"](x) * expand_max_param
        x = self.point_linear["conv"](x) * expand_max_param
        x = self.point_linear["bn"](x) * expand_max_param
        return x
...
class DynamicMBConvLayerMixture(MyModule):
                ...
                OrderedDict(
                    [
                        (
                            "conv",
                            DynamicOutConv2dMixture(
                                max(self.in_channel_list), self.mid_channel_list_options, use_we_v2=use_we_v2
                            ),
                        ),
                        ("bn", DynamicBatchNorm2dMixture(
                            self.mid_channel_list_options, use_we_v2=use_we_v2)),
                        ("act", build_activation(self.act_func)),
                    ]
                )
            )
            ...
            OrderedDict(
                [
                    (
                        "conv",
                        DynamicInSeparableConv2dMixture(
                            self.mid_channel_list_options, self.kernel_size_list, self.stride, use_we_v2=use_we_v2
                        ),
                    ),
                    ("bn", DynamicBatchNorm2dMixture(
                        self.mid_channel_list_options, use_we_v2=use_we_v2)),
            ...
            OrderedDict(
                [
                    (
                        "conv",
                        DynamicInConv2dMixture(
                            self.mid_channel_list_options, max(self.out_channel_list), use_we_v2=use_we_v2),
                    ),
    ...
    def forward(self, x, expand_ratio_weights, kernel_size_weights):
        if self.inverted_bottleneck is not None:
