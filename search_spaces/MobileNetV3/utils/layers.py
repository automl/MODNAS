...
class ResidualBlock(MyModule):
    ...
    def forward(self, x, kernel_max_param=None, expand_max_param=None):
        if self.conv is None or isinstance(self.conv, ZeroLayer):
            res = x
        elif self.shortcut is None or isinstance(self.shortcut, ZeroLayer):
            if kernel_max_param is not None:
                res = self.conv(x, kernel_max_param, expand_max_param)
            else:
                res = self.conv(x)
        else:
            if kernel_max_param is not None:
                res = self.conv(x, kernel_max_param, expand_max_param) + self.shortcut(x)
            else:
...
class ResidualBlockMix(MyModule):
    def __init__(self, conv, shortcut):
        super(ResidualBlockMix, self).__init__()
    ...
    def forward(self, x, expand_ratio_weights, kernel_size_weights):
        if self.conv is None or isinstance(self.conv, ZeroLayer):
            ...
            res = self.conv(x, expand_ratio_weights, kernel_size_weights)
        else:
            res = self.conv(x) + self.shortcut(x)
        return res
    ...
    def config(self):
        return {
            "name": ResidualBlockMix.__name__,
            "conv": self.conv.config if self.conv is not None else None,
            "shortcut": self.shortcut.config if self.shortcut is not None else None,
        }
    ...
    def build_from_config(config):
        ...
        conv = set_layer_from_config(conv_config)
        shortcut = set_layer_from_config(config["shortcut"])
        return ResidualBlockMix(conv, shortcut)
