...
from search_spaces.hat.fairseq import bleu, checkpoint_utils, options, progress_bar, tasks, utils
from search_spaces.hat.fairseq.meters import StopwatchMeter, TimeMeter
import sys
import pdb
import numpy as np
...
def get_hw_embed_dict(task):
    pickle_path = "pretrained_hpns/hat_lats_vector_dict_"+ task +".pkl"
...
def main(args):
    ...
    for model in models:
        ...
        metahpn = MetaHyperNetwork(search_space, num_random_devices=50)
        w_embed = get_hw_embed_dict(args.task_type)[args.device]
        if args.task_type == "wmt14.en-de":
            state = torch.load("path/to/en-de-checkpoint/checkpoint.pt", map_location="cpu")["hypernetwork"]
        elif args.task_type == "wmt14.en-fr":
            state = torch.load("path/to/en-fr-checkpoint/checkpoint.pt", map_location="cpu")["hypernetwork"]
        metahpn.load_state_dict(state)
        metahpn.cuda()
        metahpn.eval()
        from utils import circle_points
        scalarization = circle_points(24)
        scalarization = torch.tensor(scalarization[args.scalarization]).unsqueeze(0).cuda()
        arch_params = metahpn(scalarization, torch.tensor(w_embed).unsqueeze(0).cuda())
        arch_params_dict = convert_arch_params_to_dict(arch_params)
        arch_params_max = {}
        for k, v in arch_params_dict.items():
            arch_params_max[k] = torch.zeros_like(v)
            argmax = torch.argmax(v, dim=-1)
            arch_params_max[k][...,argmax] = 1.0
        arch_params_dict = arch_params_max

        #print(arch_params_dict)
        model.set_sample_config(search_space, arch_params_dict)
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ...
            gen_timer.start()
            hypos, decoder_times = task.inference_step(generator, models, arch_params_dict, sample, prefix_tokens)
...
def cli_main():
    ...
    parser.add_argument('--scalarization', default=0, type=int)
    parser.add_argument('--device', default="cpu_xeon", type=str)
    parser.add_argument('--task_type', default="wmt14.en-de", type=str)
