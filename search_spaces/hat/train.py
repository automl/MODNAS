...
def load_pretrained_hypernetwork(hypernetwork, task):
    ckpt_path = "hypernetwork_data_utils/hat/metahpn_hat_"+ task +".pt"
    ckpt = torch.load(ckpt_path, map_location=torch.device('cpu'))
    hypernetwork.load_state_dict(ckpt)
...
def main(args, init_distributed=False):
    ...
    hypernetwork = MetaHyperNetwork(search_space)
    # get datapaths last part
    task_name = args.data.split('/')[-1]
    load_pretrained_hypernetwork(hypernetwork, task_name)
    hw_embed_dict = get_hw_embed_dict(task_name)
    ...
    # Build trainer
    trainer = Trainer(args, task, model, hypernetwork, criterion, hw_embed_dict)
