...
from  search_spaces.hat.fairseq import distributed_utils
...
class tqdm_progress_bar(progress_bar):
    ...
    def log(self, stats, tag='', step=None):
        """Log intermediate stats according to log_interval."""
        postfix = self._format_stats(stats)
        postfix_concise = {}
        for k in postfix.keys():
            if k in ['loss', 'num_updates', 'lr', 'wps']:
                postfix_concise[k] = postfix[k]

        self.tqdm.set_postfix(postfix_concise, refresh=False)

    def print(self, stats, tag='', step=None):
        """Print end-of-epoch stats."""
        postfix = self._str_pipes(self._format_stats(stats))
        print('{} | {}'.format(self.tqdm.desc, postfix))
