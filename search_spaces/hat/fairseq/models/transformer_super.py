...
from search_spaces.hat.fairseq import options, utils
from search_spaces.hat.fairseq.models import (
    FairseqEncoder,
...
from search_spaces.hat.fairseq.modules import (
    AdaptiveSoftmax,
    PositionalEmbedding,
...
import search_spaces.hat.fairseq.init as init
...
@register_model('transformersuper2')
class TransformerSuperModel(SuperFairseqEncoderDecoderModel):
    ...
    def get_sampled_params_numel(self, choices:dict, arch_param:dict):
        self.set_sample_config(choices, arch_param)
        numels = []
        arch_encoder_layer_num = arch_param['encoder-layer-num']
        selected_id = torch.argmax(arch_encoder_layer_num).item()
        encoder_layer_num = choices['encoder-layer-num-choice'][selected_id]
        arch_decoder_layer_num = arch_param['decoder-layer-num']
        selected_id = torch.argmax(arch_decoder_layer_num).item()
        decoder_layer_num = choices['decoder-layer-num-choice'][selected_id]
        for name, module in self.named_modules():
            if hasattr(module, 'calc_sampled_param_num'):
                # a hacky way to skip the layers that exceed encoder-layer-num or decoder-layer-num
                if name.split('.')[0] == 'encoder' and eval(name.split('.')[2]) >= encoder_layer_num:
                    continue
                if name.split('.')[0] == 'decoder' and eval(name.split('.')[2]) >= decoder_layer_num:
                    continue
    ...
    def load_predictor(self, args):
        task = args.data.split('/')[-1]
        if task == "wmt16_en_de":
            task = "wmt14_en_de"
        ckpt_path   = "predictor_data_utils/hat/"+task+"_one_hot_"+".pt"
...
class TransformerEncoder(FairseqEncoder):
    ...
    def set_sample_config(self, choices:dict, arch_param:dict):
        arch_encoder_embed = arch_param['encoder-embed-dim']
        selected_id = torch.argmax(arch_encoder_embed).item()
        self.sample_embed_dim = choices['encoder-embed-choice'][selected_id]
        arch_encoder_ffn_embed_dim = arch_param['encoder-ffn-embed-dim']
        selected_id = [torch.argmax(arch_encoder_ffn_embed_dim[i]).item() for i in range(arch_encoder_ffn_embed_dim.shape[0])]
        # Caution: this is a list for all layers
        self.sample_ffn_embed_dim = [choices['encoder-ffn-embed-dim-choice'][selected_id[i]] for i in range(len(selected_id))]
        arch_encoder_layer_num = arch_param['encoder-layer-num']
        #print(arch_encoder_layer_num)
        #print(choices["encoder-layer-num-choice"])
        selected_id = torch.argmax(arch_encoder_layer_num).item()
        self.sample_layer_num = choices['encoder-layer-num-choice'][selected_id]

        # Caution: this is a list for all layers
        arch_encoder_self_attention_heads = arch_param['encoder-self-attention-heads']
        selected_id = [torch.argmax(arch_encoder_self_attention_heads[i]).item() for i in range(arch_encoder_self_attention_heads.shape[0])]
        self.sample_self_attention_heads = [choices['encoder-self-attention-heads-choice'][selected_id[i]] for i in range(len(selected_id))]
    ...
    def forward(self, src_tokens, src_lengths, arch_param:dict):
        """
        Args:
        ...
        x = self.sample_embed_scale * self.embed_tokens(src_tokens, arch_param, part='encoder')
        if self.embed_positions is not None:
            positions = self.embed_positions(src_tokens)

            # sample the positional embedding and add
            arch_param_embed_dim = arch_param["encoder-embed-dim"]
            selected_id = torch.argmax(arch_param_embed_dim).item()
            weight = arch_param_embed_dim[selected_id]
            x += positions[..., :self.sample_embed_dim]*weight
        ...
        for i, layer in enumerate(self.layers):
            # print(x.shape)
            x = layer(x, encoder_padding_mask, arch_param, idx=i)
            all_x.append(x)
        # num_layer selected 
        num_layer_selected = torch.argmax(arch_param['encoder-layer-num']).item()
        weight = arch_param['encoder-layer-num'][num_layer_selected]
        x = x*weight
        if self.layer_norm:
            x = self.layer_norm(x, arch_param, part='encoder')
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def set_sample_config(self, choices:dict, arch_param:dict):
        arch_decoder_embed = arch_param['decoder-embed-dim']
        selected_id = torch.argmax(arch_decoder_embed).item()
        self.sample_embed_dim = choices['decoder-embed-choice'][selected_id]
        arch_encoder_embed = arch_param['encoder-embed-dim']
        self.sample_encoder_embed_dim = choices['encoder-embed-choice'][torch.argmax(arch_encoder_embed).item()]

        # Caution: this is a list for all layers
        arch_decoder_ffn_embed_dim = arch_param['decoder-ffn-embed-dim']
        selected_id = [torch.argmax(arch_decoder_ffn_embed_dim[i]).item() for i in range(arch_decoder_ffn_embed_dim.shape[0])]
        self.sample_ffn_embed_dim = [choices['decoder-ffn-embed-dim-choice'][selected_id[i]] for i in range(len(selected_id))]

        # Caution: this is a list for all layers
        arch_decoder_self_attention_heads = arch_param['decoder-self-attention-heads']
        selected_id = [torch.argmax(arch_decoder_self_attention_heads[i]).item() for i in range(arch_decoder_self_attention_heads.shape[0])]
        self.sample_self_attention_heads = [choices['decoder-self-attention-heads-choice'][selected_id[i]] for i in range(len(selected_id))]

        # Caution: this is a list for all layers
        arch_decoder_ende_attention_heads = arch_param['decoder-ende-attention-heads']
        self.sample_ende_attention_heads = [choices['decoder-ende-attention-heads-choice'][torch.argmax(arch_decoder_ende_attention_heads[i]).item()] for i in range(arch_decoder_ende_attention_heads.shape[0])]
        # Caution: this is a list for all layers
        arch_decoder_arbitrary_ende_attn = arch_param['decoder-arbitrary-ende-attn']
        self.sample_arbitrary_ende_attn = [choices['decoder-arbitrary-ende-attn-choice'][torch.argmax(arch_decoder_arbitrary_ende_attn[i]).item()] for i in range(arch_decoder_arbitrary_ende_attn.shape[0])]
        decoder_layer_num = arch_param['decoder-layer-num']
        self.sample_layer_num = choices['decoder-layer-num-choice'][torch.argmax(decoder_layer_num).item()]
    ...
    def forward(self, prev_output_tokens, arch_param, encoder_out=None, incremental_state=None, **unused):
        """
        Args:
        ...
        x, extra = self.extract_features(prev_output_tokens, arch_param, encoder_out, incremental_state)
        x = self.output_layer(x, arch_param)
        return x, extra

    def extract_features(self, prev_output_tokens, arch_param, encoder_out=None, incremental_state=None, **unused):
        """
        ...
        if positions is not None:
            embed_dim_arch = arch_param['decoder-embed-dim']
            selected_id = torch.argmax(embed_dim_arch).item()
            weight = embed_dim_arch[selected_id]
            positions = positions[..., :self.sample_embed_dim] * weight
        ...
        # embed tokens and positions
        x = self.sample_embed_scale * self.embed_tokens(prev_output_tokens, arch_param, part='decoder')
        ...
        for i, layer in enumerate(self.layers):
            ...
            # selected arbirary ende attn head
            arch_param_arbitrary_ende_attn_i = arch_param['decoder-arbitrary-ende-attn'][i]
            selected_id = torch.argmax(arch_param_arbitrary_ende_attn_i).item()
            weight = arch_param_arbitrary_ende_attn_i[selected_id]
            if encoder_out_feed is not None:
                encoder_out_feed = encoder_out_feed*weight
            if encoder_padding_mask_feed is not None:
                encoder_padding_mask_feed = encoder_padding_mask_feed#*weight
        ...
        num_layer_selected = torch.argmax(arch_param['decoder-layer-num']).item()
        weight = arch_param['decoder-layer-num'][num_layer_selected]
        x = x*weight
        if self.layer_norm:
            x = self.layer_norm(x, arch_param, part='decoder')

        # T x B x C -> B x T x C
        x = x.transpose(0, 1)

        if self.project_out_dim is not None:
            #print("project_out_dim")
            #print(self.project_out_dim)
            #print(x.shape)
            #print(self.project_out_dim.weight.shape)
            x = self.project_out_dim(x)
        if not self.get_attn:
            attns = attns[-1]
        return x, {'attn': attns, 'inner_states': inner_states}

    def output_layer(self, features, arch_param, **kwargs):
        """Project features to the vocabulary size."""
        arch_param_embed = arch_param['decoder-embed-dim']
        selected_id = torch.argmax(arch_param_embed).item()
        weight = arch_param_embed[selected_id]
        if self.adaptive_softmax is None:
            # project back to size of vocabulary
            if self.share_input_output_embed:
                return F.linear(features, self.embed_tokens.sampled_weight('decoder')) * weight
            else:
                return F.linear(features, self.embed_out[:, :self.sample_embed_dim]) * weight
        else:
            return features
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask, arch_param, attn_mask=None, idx=None):
        """
        Args:
        ...
        if self.is_identity_layer:
            return x
        residual = x
        x = self.maybe_layer_norm(self.self_attn_layer_norm, x, arch_param, before=True)
        ...
        x, _ = self.self_attn(query=x, key=x, value=x, arch_param=arch_param, key_padding_mask=encoder_padding_mask, part= "encoder", idx=idx)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x[:residual.size(0),:,:] = residual + x[:residual.size(0),:,:]
        x = self.maybe_layer_norm(self.self_attn_layer_norm, x, arch_param, after=True)

        residual = x
        x = self.maybe_layer_norm(self.final_layer_norm, x, arch_param, before=True)
        x = self.activation_fn(self.fc1(x, arch_param, part="encoder", idx=idx))
        x = F.dropout(x, p=self.sample_activation_dropout, training=self.training)
        x = self.fc2(x, arch_param, part = "encoder", idx=idx)
        x = F.dropout(x, p=self.sample_dropout, training=self.training)
        x = residual + x
        x = self.maybe_layer_norm(self.final_layer_norm, x, arch_param, after=True)
        return x

    def maybe_layer_norm(self, layer_norm, x, arch_param, before=False, after=False):
        assert before ^ after
        if after ^ self.normalize_before:
            return layer_norm(x, arch_param, part="encoder")
        else:
            return x
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
        ...
        if self.encoder_attn is not None:
            residual = x
            x = self.maybe_layer_norm(self.encoder_attn_layer_norm, x, arch_param, before=True)
            ...
            x = F.dropout(x, p=self.sample_dropout, training=self.training)
            x = residual + x
            x = self.maybe_layer_norm(self.encoder_attn_layer_norm, x, arch_param, after=True)

        residual = x
        x = self.maybe_layer_norm(self.final_layer_norm, x, arch_param, before=True)
        x = self.activation_fn(self.fc1(x, arch_param, part = 'decoder', idx=idx))
        x = F.dropout(x, p=self.sample_activation_dropout, training=self.training)
        x = self.fc2(x, arch_param, part='decoder', idx=idx)
        x = F.dropout(x, p=self.sample_dropout, training=self.training)
        x = residual + x
        x = self.maybe_layer_norm(self.final_layer_norm, x, arch_param, after=True)
    ...
    def maybe_layer_norm(self, layer_norm, x, arch_param, before=False, after=False):
        assert before ^ after
        if after ^ self.normalize_before:
            return layer_norm(x, arch_param, part='decoder')
        else:
            return x
...
@register_model_architecture('transformersuper2', 'transformersuper2')
...
@register_model_architecture('transformersuper2', 'transformersuper_iwslt_de_en2')
def transformer_iwslt_de_en2(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
...
@register_model_architecture('transformersuper2', 'transformersuper_wmt_en_de2')
def transformer_wmt_en_de2(args):
    base_architecture(args)

@register_model_architecture('transformersuper2', 'transformersuper_wmt_en_fr2')
def transformer_wmt_en_fr2(args):
    base_architecture(args)

# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture('transformersuper2', 'transformersuper_vaswani_wmt_en_de_big2')
def transformer_vaswani_wmt_en_de_big2(args):
...
@register_model_architecture('transformersuper2', 'transformersuper_vaswani_wmt_en_fr_big2')
def transformer_vaswani_wmt_en_fr_big2(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_vaswani_wmt_en_de_big2(args)


@register_model_architecture('transformersuper2', 'transformersuper_wmt_en_de_big2')
def transformer_wmt_en_de_big2(args):
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    transformer_vaswani_wmt_en_de_big2(args)


# default parameters used in tensor2tensor implementation
@register_model_architecture('transformersuper2', 'transformersuper_wmt_en_de_big_t2t2')
def transformer_wmt_en_de_big_t2t2(args):
