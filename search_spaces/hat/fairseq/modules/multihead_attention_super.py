...
class MultiheadAttentionSuper(nn.Module):
    ...
    def forward(self, query, key, value, arch_param, idx, key_padding_mask=None, incremental_state=None,
                need_weights=True, static_kv=False, attn_mask=None, part='encoder'):
        ...
        if part == "encoder":
            ...
            seleced_id = torch.argmax(arch_param_embed_dim).item()
            weight1 = arch_param_embed_dim[seleced_id]
            arch_param_num_heads = arch_param["encoder-self-attention-heads"][idx]
            seleced_id = torch.argmax(arch_param_num_heads).item()
            weight2 = arch_param_num_heads[seleced_id]
        elif part == "decoder":
            arch_param_embed_dim = arch_param["decoder-embed-dim"]
            seleced_id = torch.argmax(arch_param_embed_dim).item()
            weight1 = arch_param_embed_dim[seleced_id]
            arch_param_num_heads = arch_param["decoder-self-attention-heads"][idx]
            seleced_id = torch.argmax(arch_param_num_heads).item()
            weight2 = arch_param_num_heads[seleced_id]
        elif part == "decoder_ende":
            arch_param_embed_dim = arch_param["decoder-embed-dim"]
            seleced_id = torch.argmax(arch_param_embed_dim).item()
            weight1 = arch_param_embed_dim[seleced_id]
            arch_param_num_heads = arch_param["decoder-ende-attention-heads"][idx]
            seleced_id = torch.argmax(arch_param_num_heads).item()
            weight2 = arch_param_num_heads[seleced_id]

        tgt_len, bsz, embed_dim = query.size()
        #print("num_heads", weight1)
        #print("embed_dim", weight2)
        ...
        if self.self_attention:
            # self-attention
            q, k, v = self.in_proj_qkv(query, weight1)
        elif self.encoder_decoder_attention:
            # encoder-decoder attention
            q = self.in_proj_q(query, weight1)
            if key is None:
                assert value is None
                k = v = None
            else:
                k = self.in_proj_k(key, weight1)
                v = self.in_proj_v(key, weight1)

        else:
            q = self.in_proj_q(query, weight1)
            k = self.in_proj_k(key, weight1)
            v = self.in_proj_v(value, weight1)
        ...
        attn = self.out_proj(attn, arch_param, idx)
        ...
        if need_weights:
            ...
            attn_weights*weight2
        else:
            attn_weights = None

        return attn*weight2, attn_weights

    def in_proj_qkv(self, query, weight):
        return self._in_proj(query, sample_dim=self.sample_q_embed_dim, w=weight).chunk(3, dim=-1) 

    def in_proj_q(self, query, w):
        if self.qkv_same_dim:
            return self._in_proj(query, end=self.qkv_dim, sample_dim=self.sample_q_embed_dim, w=w)
        else:
            bias = self.in_proj_bias
            if bias is not None:
                bias = bias[:self.qkv_dim] 
            return F.linear(query, self.q_proj_weight[..., :self.sample_q_embed_dim], bias) * w

    def in_proj_k(self, key, w):
        if self.qkv_same_dim:
            return self._in_proj(key, start=self.qkv_dim, end=2 * self.qkv_dim, sample_dim=self.sample_kv_embed_dim, w=w)
        else:
            weight = self.k_proj_weight
            bias = self.in_proj_bias
            if bias is not None:
                bias = bias[self.qkv_dim:2 * self.qkv_dim]
            return F.linear(key, weight[..., :self.sample_kv_embed_dim], bias) * w

    def in_proj_v(self, value, w):
        if self.qkv_same_dim:
            return self._in_proj(value, start=2 * self.qkv_dim, sample_dim=self.sample_kv_embed_dim, w=w)
        else:
            weight = self.v_proj_weight
            bias = self.in_proj_bias
            if bias is not None:
                bias = bias[2 * self.qkv_dim:]
            return F.linear(value, weight[..., :self.sample_kv_embed_dim], bias) * w

    def _in_proj(self, input, sample_dim, start=0, end=None, w=None):
        weight = self.in_proj_weight
        bias = self.in_proj_bias
        weight = weight[start:end, :sample_dim]
        if bias is not None:
            bias = bias[start:end]
        return F.linear(input, weight, bias) * w
