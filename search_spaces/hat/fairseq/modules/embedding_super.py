...
class EmbeddingSuper(nn.Embedding):
    ...
    def forward(self, input, arch_param, part='encoder'):
        if part == "encoder":
        ...
        #print("weight", weight)
        return F.embedding(input, self.sampled_weight(part), self.padding_idx, self.max_norm, self.norm_type, self.scale_grad_by_freq, self.sparse) * weight
