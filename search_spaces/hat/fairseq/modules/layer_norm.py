...
class LayerNormSuper(torch.nn.LayerNorm):
    ...
    def forward(self, x, arch_param, part='encoder'):
        if part == "encoder":
            arch_param_encoder_dim = arch_param["encoder-embed-dim"]
            seleced_id = torch.argmax(arch_param_encoder_dim).item()
            weight = arch_param_encoder_dim[seleced_id]
        elif part == "decoder":
            arch_param_decoder_dim = arch_param["decoder-embed-dim"]
            seleced_id = torch.argmax(arch_param_decoder_dim).item()
            weight = arch_param_decoder_dim[seleced_id]
        self.sample_parameters()
        self.sample_embed_dim = self.samples['weight'].shape[-1]
        return F.layer_norm(x, (self.sample_embed_dim,), weight=self.samples['weight'], bias=self.samples['bias'], eps=self.eps)*weight
