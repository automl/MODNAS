...
from  search_spaces.hat.fairseq.modules import gelu, gelu_accurate
...
def handle_save_path(args):
    if args.save_dir is None:
        ...
        args.save_dir = os.path.join('checkpoints2', dirname)
...
def conver_search_space_to_id_map(search_space_dict):
    id_map ={}
...
def get_config_features_one_hot(config, search_space):

    features = []
    search_space_choices = get_space(search_space)
    #search_space_id_map = conver_search_space_to_id_map(search_space_choices)
    max_enc_layer = max(search_space_choices["encoder-layer-num-choice"])
    max_dec_layer = max(search_space_choices["decoder-layer-num-choice"])
    features_encode_embed = [0 for _ in range(len(search_space_choices["encoder-embed-choice"]))]
    features_decode_embed = [0 for _ in range(len(search_space_choices["decoder-embed-choice"]))]
    features_encode_ffn_embed_dim = [0 for _ in range(max_enc_layer*len(search_space_choices["encoder-ffn-embed-dim-choice"]))]
    features_decode_ffn_embed_dim = [0 for _ in range(max_dec_layer*len(search_space_choices["decoder-ffn-embed-dim-choice"]))]
    features_encode_layer_num = [0 for _ in range(len(search_space_choices["encoder-layer-num-choice"]))]
    features_decode_layer_num = [0 for _ in range(len(search_space_choices["decoder-layer-num-choice"]))]
    features_encode_self_attention_heads = [0 for _ in range(max_enc_layer*len(search_space_choices["encoder-self-attention-heads-choice"]))]
    features_decode_self_attention_heads = [0 for _ in range(max_dec_layer*len(search_space_choices["decoder-self-attention-heads-choice"]))]
    features_decode_ende_attention_heads = [0 for _ in range(max_dec_layer*len(search_space_choices["decoder-ende-attention-heads-choice"]))]
    features_decode_arbitrary_ende_attn = [0 for _ in range(max_dec_layer*len(search_space_choices["decoder-arbitrary-ende-attn-choice"]))]
    for i,choice in enumerate(search_space_choices["encoder-embed-choice"]):
        if config['encoder_embed_dim'] == choice:
            features_encode_embed[i] = 1
            break
    for i,choice in enumerate(search_space_choices["decoder-embed-choice"]):
        if config['decoder_embed_dim'] == choice:
            features_decode_embed[i] = 1
            break
    for i,choice in enumerate(search_space_choices["encoder-layer-num-choice"]):
        if config['encoder_layer_num'] == choice:
            features_encode_layer_num[i] = 1
            break
    for i,choice in enumerate(search_space_choices["decoder-layer-num-choice"]):
        if config['decoder_layer_num'] == choice:
            features_decode_layer_num[i] = 1
            break
    start_enc_ffn_embed_dim = 0
    start_enc_self_attention_heads = 0
    # replace "-" with "_" in feature names
    new_dict = {}
    for k, v in config.items():
        new_dict[k.replace("-", "_")] = v
    config = new_dict
    for d in range(config['encoder_layer_num']):
        for i,choice in enumerate(search_space_choices["encoder-ffn-embed-dim-choice"]):
            if config['encoder_ffn_embed_dim'][d] == choice:
                features_encode_ffn_embed_dim[start_enc_ffn_embed_dim+i] = 1
                break
        for i,choice in enumerate(search_space_choices["encoder-self-attention-heads-choice"]):
            if config['encoder_self_attention_heads'][d] == choice:
                features_encode_self_attention_heads[start_enc_self_attention_heads+i] = 1
                break
        start_enc_ffn_embed_dim += len(search_space_choices["encoder-ffn-embed-dim-choice"])
        start_enc_self_attention_heads += len(search_space_choices["encoder-self-attention-heads-choice"])
    start_dec_ffn_embed_dim = 0
    start_dec_self_attention_heads = 0
    start_dec_ende_attention_heads = 0
    start_dec_arbitrary_ende_attn = 0
    for d in range(config['decoder_layer_num']):
        for i,choice in enumerate(search_space_choices["decoder-ffn-embed-dim-choice"]):
            if config['decoder_ffn_embed_dim'][d] == choice:
                features_decode_ffn_embed_dim[start_dec_ffn_embed_dim+i] = 1
                break
        for i,choice in enumerate(search_space_choices["decoder-self-attention-heads-choice"]):
            if config['decoder_self_attention_heads'][d] == choice:
                features_decode_self_attention_heads[start_dec_self_attention_heads+i] = 1
                break
        for i,choice in enumerate(search_space_choices["decoder-ende-attention-heads-choice"]):
            if config['decoder_ende_attention_heads'][d] == choice:
                features_decode_ende_attention_heads[start_dec_ende_attention_heads+i] = 1
                break
        for i,choice in enumerate(search_space_choices["decoder-arbitrary-ende-attn-choice"]):
            if config['decoder_arbitrary_ende_attn'][d] == choice:
                features_decode_arbitrary_ende_attn[start_dec_arbitrary_ende_attn+i] = 1
                break
        start_dec_ffn_embed_dim += len(search_space_choices["decoder-ffn-embed-dim-choice"])
        start_dec_self_attention_heads += len(search_space_choices["decoder-self-attention-heads-choice"])
        start_dec_ende_attention_heads += len(search_space_choices["decoder-ende-attention-heads-choice"])
        start_dec_arbitrary_ende_attn += len(search_space_choices["decoder-arbitrary-ende-attn-choice"])

    
    features.extend(features_encode_embed)
    features.extend(features_decode_embed)
    features.extend(features_encode_layer_num)
    features.extend(features_decode_layer_num)
    features.extend(features_encode_ffn_embed_dim)
    features.extend(features_decode_ffn_embed_dim)
    features.extend(features_encode_self_attention_heads)
    features.extend(features_decode_self_attention_heads)
    features.extend(features_decode_ende_attention_heads)
    features.extend(features_decode_arbitrary_ende_attn)
    #print(len(features))
    #print(features)

    return features

def get_config_features_one_hot_from_vec(feature_vec, search_space):
    #print(feature_vec)
    encoder_embed_dim = feature_vec[0]
    encoder_layer_num = feature_vec[1]
    encoder_ffn_embed_dim = feature_vec[2:2+encoder_layer_num]
    encoder_self_attention_heads = feature_vec[2+encoder_layer_num:2+encoder_layer_num*2]
    decoder_embed_dim = feature_vec[2+encoder_layer_num*2]
    decoder_layer_num = feature_vec[2+encoder_layer_num*2+1]
    decoder_ffn_embed_dim = feature_vec[2+encoder_layer_num*2+2:2+encoder_layer_num*2+2+decoder_layer_num]
    decoder_self_attention_heads = feature_vec[2+encoder_layer_num*2+2+decoder_layer_num:2+encoder_layer_num*2+2+decoder_layer_num*2]
    decoder_ende_attention_heads = feature_vec[2+encoder_layer_num*2+2+decoder_layer_num*2:2+encoder_layer_num*2+2+decoder_layer_num*3]
    decoder_arbitrary_ende_attn = feature_vec[2+encoder_layer_num*2+2+decoder_layer_num*3:2+encoder_layer_num*2+2+decoder_layer_num*4]
    ...
    for i,choice in enumerate(search_space_choices["encoder-embed-choice"]):
        if encoder_embed_dim == choice:
            features_encode_embed[i] = 1
            break
    for i,choice in enumerate(search_space_choices["decoder-embed-choice"]):
        if decoder_embed_dim == choice:
            features_decode_embed[i] = 1
            break
    for i,choice in enumerate(search_space_choices["encoder-layer-num-choice"]):
        if encoder_layer_num == choice:
            features_encode_layer_num[i] = 1
            break
    for i,choice in enumerate(search_space_choices["decoder-layer-num-choice"]):
        if decoder_layer_num == choice:
            features_decode_layer_num[i] = 1
            break
    start_enc_ffn_embed_dim = 0
    start_enc_self_attention_heads = 0
    for d in range(encoder_layer_num):
        for i,choice in enumerate(search_space_choices["encoder-ffn-embed-dim-choice"]):
            if encoder_ffn_embed_dim[d] == choice:
                features_encode_ffn_embed_dim[start_enc_ffn_embed_dim+i] = 1
                break
        for i,choice in enumerate(search_space_choices["encoder-self-attention-heads-choice"]):
            if encoder_self_attention_heads[d] == choice:
                features_encode_self_attention_heads[start_enc_self_attention_heads+i] = 1
                break
        start_enc_ffn_embed_dim += len(search_space_choices["encoder-ffn-embed-dim-choice"])
        start_enc_self_attention_heads += len(search_space_choices["encoder-self-attention-heads-choice"])
    ...
    for d in range(decoder_layer_num):
        for i,choice in enumerate(search_space_choices["decoder-ffn-embed-dim-choice"]):
            if decoder_ffn_embed_dim[d] == choice:
                features_decode_ffn_embed_dim[start_dec_ffn_embed_dim+i] = 1
                break
        for i,choice in enumerate(search_space_choices["decoder-self-attention-heads-choice"]):
            if decoder_self_attention_heads[d] == choice:
                features_decode_self_attention_heads[start_dec_self_attention_heads+i] = 1
                break
        for i,choice in enumerate(search_space_choices["decoder-ende-attention-heads-choice"]):
            if decoder_ende_attention_heads[d] == choice:
                features_decode_ende_attention_heads[start_dec_ende_attention_heads+i] = 1
                break
        for i,choice in enumerate(search_space_choices["decoder-arbitrary-ende-attn-choice"]):
            if decoder_arbitrary_ende_attn[d] == choice:
                features_decode_arbitrary_ende_attn[start_dec_arbitrary_ende_attn+i] = 1
                break
        start_dec_ffn_embed_dim += len(search_space_choices["decoder-ffn-embed-dim-choice"])
        start_dec_self_attention_heads += len(search_space_choices["decoder-self-attention-heads-choice"])
