...
from predictors.help.utils import *
...
class Data:
    ...
    def sample_device_embedding(self, mode="meta_train"):
        if mode == 'meta_train':
            devices = self.meta_train_devices + self.meta_valid_devices
        elif mode == 'meta_test':
            devices = self.meta_test_devices
        elif mode == 'all':
            devices = self.meta_train_devices + self.meta_valid_devices + self.meta_test_devices
        device = np.random.choice(devices, size = 1)[0]

        hw_embed = normalization(self.latency[device][self.hw_emb_idx], portion=1.0)
        return hw_embed, device

    def sample_specific_device_embedding(self, mode="meta_train", device=None):

        hw_embed = normalization(self.latency[device][self.hw_emb_idx], portion=1.0)
        return hw_embed, device
    
    def sample_all_devices(self, mode="all"):
        if mode == 'meta_train':
            devices = self.meta_train_devices + self.meta_valid_devices
        elif mode == 'meta_test':
            devices = self.meta_test_devices
        ...
        hw_embeddings = [
            normalization(
                self.latency[device][self.hw_emb_idx],
                portion=1.0
            ) for device in devices
        ]
        return hw_embeddings, devices
