...
class PredictorNB201(nn.Module):
    def __init__(self):
        super(PredictorNB201, self).__init__()
        self.embedding_dim = 256
        self.num_ops = 5
        self.num_edges = 6
        self.embedding_mat_ops_k = torch.nn.Embedding(self.num_ops, self.embedding_dim).cuda()
        self.embedding_mat_ops_q = torch.nn.Embedding(self.num_ops, self.embedding_dim).cuda()
        self.embedding_mat_ops_v = torch.nn.Embedding(self.num_ops, self.embedding_dim).cuda()
        self.embedding_mat_edges = torch.nn.Embedding(self.num_edges, self.embedding_dim).cuda()
        self.linear1 = nn.Linear(self.embedding_dim, 150)
    ...
    def forward(self, x):
        x = x.reshape(1, 6,5)
        argmax= torch.argmax(x, dim=-1).cuda()
        k = self.embedding_mat_ops_k(argmax)
        q = self.embedding_mat_ops_q(argmax)
        v = self.embedding_mat_ops_v(argmax)
        x = torch.matmul(k,q.transpose(1,2))
        x = F.softmax(x, dim=-1)
        x = torch.matmul(x,v)
        edges = torch.tensor([0,1,2,3,4,5]).unsqueeze(0).cuda()
        edges_embed = self.embedding_mat_edges(edges)
        x = x*edges_embed
        x = torch.sum(x, dim=1)
        x = torch.squeeze(x)
        x = F.relu(self.linear1(x.reshape(1, -1)))
...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat=8, hw_embed_on=False, hw_embed_dim=None, layer_size=128):
...
class GCNHardware(nn.Module):
    """
    The base model for MAML (Meta-SGD) for meta-NAS-predictor.
    """

    def __init__(self, nfeat, hw_embed_on, hw_embed_dim, layer_size):
        super(GCNHardware, self).__init__()
        self.hw_embed_on = hw_embed_on
        self.layer_size = layer_size
